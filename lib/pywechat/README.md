...
### 与 OpenClaw 配合（监听推送 Webhook）

若希望由 **OpenClaw** 理解微信消息语义并做高级业务处理（如转发到供应商群、调用会员管理技能等），可使用脚本 **`scripts/wechat_listener_webhook.py`**：在后台持续监听配置中的微信群，**仅将新消息推送到 OpenClaw 的 Webhook**，不做本地自动回复。配置见 **`listener_config.example.yaml`**（复制为 `listener_config.yaml` 后填写 `openclaw_webhook_url`、`openclaw_token` 和 `groups`）。发消息能力由 OpenClaw 的 wechat-skill 提供（见 `C:\Users\neo\.openclaw\workspace\skills\wechat-skill`）。
